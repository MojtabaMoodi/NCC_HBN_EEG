...
conda run -n eeg_env --no-capture-output python user_identification/train_labram_arcface.py \
  ...
  --output_dir final_user_identification/4s/fold_3 \
  --segment_length 4s \
  --epochs 200 \
  --batch_size 384 \
  --seed 42 --resume final_user_identification/4s/fold_3/best_model.pth &> final_user_identification/4s/fold_3/train_fold3.log
