#!/bin/bash
# Submit frozen LaBraM jobs for 2s segments only (age, gender, regression).
set -euo pipefail
...
for TASK in age gender regression; do
  if [[ "${TASK}" == "regression" ]]; then
    JOB_NAME="labram_age_regression_frozen_2s"
    RUN_DIR="${EEG_ROOT}/final_logs_frozen/LaBraM/labram_age_regression_frozen_2s"
  else
    JOB_NAME="labram_${TASK}_frozen_2s"
    RUN_DIR="${EEG_ROOT}/final_logs_frozen/LaBraM/labram_${TASK}_frozen_2s"
  fi
  ...
  sbatch \
    ...
    --export=ALL,TASK="${TASK}",SEGMENT=2s,EEG_ROOT="${EEG_ROOT}",DATA_PATH="${DATA_PATH}" \
    "${SLURM_SCRIPT}"
done

echo "Submitted 3 frozen 2s jobs."
